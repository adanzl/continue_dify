...
exports.createRuleBlock = void 0;
const builtIn_1 = require("../builtIn");
const NAME_ARG_DESC = "Short, descriptive name summarizing the rule's purpose (e.g. 'React Standards', 'Type Hints')";
const RULE_ARG_DESC = "Clear, imperative instruction for future code generation (e.g. 'Use named exports', 'Add Python type hints'). Each rule should focus on one specific standard.";
const DESC_ARG_DESC = "Description of when this rule should be applied. Required for Agent Requested rules (AI decides when to apply). Optional for other types.";
const GLOB_ARG_DESC = "Optional file patterns to which this rule applies (e.g. ['**/*.{ts,tsx}'] or ['src/**/*.ts', 'tests/**/*.ts'])";
const REGEX_ARG_DESC = "Optional regex patterns to match against file content. Rule applies only to files whose content matches the pattern (e.g. 'useEffect' for React hooks or '\\bclass\\b' for class definitions)";
const ALWAYS_APPLY_DESC = "Whether this rule should always be applied. Set to false for Agent Requested and Manual rules. Omit or set to true for Always and Auto Attached rules.";
exports.createRuleBlock = {
    type: "function",
    ...
    function: {
        name: builtIn_1.BuiltInToolNames.CreateRuleBlock,
    ...
    systemMessageDescription: {
        prefix: `Sometimes the user will provide feedback or guidance on your output. If you were not aware of these "rules", consider using the ${builtIn_1.BuiltInToolNames.CreateRuleBlock} tool to persist the rule for future interactions.
...
To create a rule, respond with a ${builtIn_1.BuiltInToolNames.CreateRuleBlock} tool call and the following arguments:
- name: ${NAME_ARG_DESC}
- rule: ${RULE_ARG_DESC}
- description: ${DESC_ARG_DESC}
- globs: ${GLOB_ARG_DESC}
