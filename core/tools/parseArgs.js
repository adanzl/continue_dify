...
exports.safeParseToolCallArgs = safeParseToolCallArgs;
exports.getStringArg = getStringArg;
exports.getOptionalStringArg = getOptionalStringArg;
exports.getNumberArg = getNumberArg;
exports.getBooleanArg = getBooleanArg;
function safeParseToolCallArgs(toolCall) {
    try {
...
function getStringArg(args, argName, allowEmpty = false) {
    if (!args || !(argName in args)) {
        throw new Error(`\`${argName}\` argument is required${allowEmpty ? "" : " and must not be empty or whitespace-only"}. (type string)`);
    }
...
function getOptionalStringArg(args, argName, allowEmpty = false) {
    if (typeof args?.[argName] === "undefined") {
        return undefined;
    }
    return getStringArg(args, argName, allowEmpty);
}
function getNumberArg(args, argName) {
    if (!args || !(argName in args)) {
...
function getBooleanArg(args, argName, required = false) {
    if (!args || !(argName in args)) {
        if (required) {
