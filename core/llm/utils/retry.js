...
function defaultOnRetry(error, attempt, delay) {
    console.warn(`Retry attempt ${attempt} after ${delay}ms delay. Error: ${error.message || error}`);
}
/**
            ...
            ? async function* (...methodArgs) {
                ...
                for (let attempt = 1; attempt <= config.maxAttempts; attempt++) {
                    try {
                        ...
                        yield* createRetryableAsyncGenerator(generator, config, originalMethod, this, methodArgs, attempt);
                        ...
                        const delay = calculateDelay(attempt, config.baseDelay, config.maxDelay, config.jitterFactor, error);
 ...
 */
async function* createRetryableAsyncGenerator(generator, config, originalMethod, context, args, initialAttempt) {
    ...
    try {
        ...
        let lastError = error;
        for (let retryAttempt = attempt + 1; retryAttempt <= config.maxAttempts; retryAttempt++) {
            // Check if we should retry this error
            ...
            // Calculate delay and wait
            const delay = calculateDelay(retryAttempt, config.baseDelay, config.maxDelay, config.jitterFactor, error);
