...
const formatMessageForO1OrGpt5 = (messages) => {
...
const formatMessageForO1OrGpt5ForResponses = (messages) => {
    const input = [];
...
class OpenAI extends index_js_1.BaseLLM {
    constructor(options) {
        super(options);
        this.useLegacyCompletionsEndpoint = undefined;
    ...
    _convertArgs(options, messages) {
        const finalOptions = (0, openaiTypeConverters_js_1.toChatBody)(messages, options, {
    ...
    _convertArgsResponses(options, messages) {
        // Specialized conversion for Responses API (strongly typed body)
        const model = options.model;
        const input = (0, openaiTypeConverters_js_1.toResponsesInput)(messages);
        const body = {
            model,
            input,
    ...
    async *_legacystreamComplete(prompt, signal, options) {
        const args = this._convertArgs(options, []);
        args.prompt = prompt;
    ...
    async *_streamChat(messages, signal, options) {
        if (!isChatOnlyModel(options.model) &&
            ...
            (NON_CHAT_MODELS.includes(options.model) ||
                ...
                options.raw)) {
            for await (const content of this._legacystreamComplete((0, messageContent_js_1.renderChatMessage)(messages[messages.length - 1]), signal, options)) {
                yield {
                    role: "assistant",
                    content,
                };
        ...
        for await (const value of (0, fetch_1.streamSse)(response)) {
            const chunk = (0, openaiTypeConverters_js_1.fromChatCompletionChunk)(value);
            if (chunk) {
                yield chunk;
            }
        }
    ...
    async *_streamResponses(messages, signal, options) {
        if (!this.isOSeriesOrGpt5Model(options.model)) {
            return;
        }
        const body = this._convertArgsResponses(options, messages);
        ...
        for await (const evt of (0, fetch_1.streamSse)(response)) {
            try {
                const msg = (0, openaiTypeConverters_js_1.fromResponsesChunk)(evt);
    ...
    async _responses(messages, signal, options) {
        if (!this.isOSeriesOrGpt5Model(options.model)) {
        ...
        const msg = (0, openaiTypeConverters_js_1.fromResponsesChunk)(data);
        if (msg)
            return msg;
        return { role: "assistant", content: "" };
    }
