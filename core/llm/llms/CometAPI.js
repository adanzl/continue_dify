...
class CometAPIAuthenticationError extends CometAPIError {
    constructor(message = "Invalid CometAPI API key") {
...
class CometAPIQuotaExceededError extends CometAPIError {
    constructor(message = "CometAPI quota exceeded") {
 ...
 */
class CometAPI extends OpenAI_js_1.default {
    constructor(options) {
        ...
        try {
            const cometProvider = llm_info_1.allModelProviders.find((p) => p.id === "cometapi");
            const info = cometProvider?.models.find((m) => m.regex ? m.regex.test(this.model) : m.model === this.model);
    ...
    static validateConfig(options) {
        ...
        if (options.apiBase && !CometAPI.isValidApiBase(options.apiBase)) {
            throw new CometAPIError(`Invalid CometAPI base URL: ${options.apiBase}. Expected https://api.cometapi.com/v1/ or compatible endpoint`);
        }
        if (options.model &&
            !CometAPI.isValidModelFormat(options.model) &&
            typeof process !== "undefined" &&
            process.env?.NODE_ENV !== "test") {
            console.warn(`CometAPI: Model "${options.model}" may not be supported. Check CometAPI documentation for available models.`);
        }
    }
    /**
     * Validate API base URL format
     */
    static isValidApiBase(apiBase) {
        try {
     ...
     */
    static isValidModelFormat(model) {
    ...
    filterChatModels(models) {
        ...
        return models.filter((model) => {
            ...
            const shouldIgnore = CometAPI.IGNORE_PATTERNS.some((pattern) => modelName.includes(pattern.toLowerCase()));
            return !shouldIgnore;
        });
    }
    /**
    ...
    async listModels() {
        try {
            ...
            const filteredModels = this.filterChatModels(allModels.map((id) => ({ id })));
            ...
            if (statusCode === 401) {
                ...
                throw new CometAPIError(`CometAPI client error: ${errorMessage}`, "CLIENT_ERROR", statusCode);
            }
    ...
    async *_streamChat(messages, signal, options = {}) {
        try {
            ...
            if (statusCode === 401) {
                ...
                throw new CometAPIError(`Model "${this.model}" is not available on CometAPI. Please check available models.`, "MODEL_NOT_FOUND", 404);
            }
            else {
                // Re-throw with more context
                throw new CometAPIError(`CometAPI chat completion failed: ${errorMessage}`, "COMPLETION_ERROR", statusCode);
            }
        }
    }
}
