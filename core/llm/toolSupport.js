...
exports.PROVIDER_TOOL_SUPPORT = {
    "continue-proxy": (model) => {
        try {
            const { provider, model: _model } = (0, config_yaml_1.parseProxyModelName)(model);
            if (provider && _model && provider !== "continue-proxy") {
                const fn = exports.PROVIDER_TOOL_SUPPORT[provider];
                if (fn) {
                    return fn(_model);
                }
            }
        ...
        return ["claude", "gpt-4", "o3", "gemini", "gemma"].some((part) => model.toLowerCase().startsWith(part));
    },
    anthropic: (model) => {
    ...
    xAI: (model) => {
        const lowerCaseModel = model.toLowerCase();
        return ["grok-3", "grok-4", "grok-4-1", "grok-code"].some((val) => lowerCaseModel.includes(val));
    },
    bedrock: (model) => {
        if (model.includes("claude-2") || model.includes("claude-instant")) {
            return false;
    ...
    ollama: (model) => {
        ...
        if (["vision", "math", "guard", "mistrallite", "mistral-openorca"].some((part) => modelName.toLowerCase().includes(part))) {
            return false;
        }
        if ([
            "cogito",
    ...
    openrouter: (model) => {
        ...
        if (["vision", "math", "guard", "mistrallite", "mistral-openorca"].some((part) => model.toLowerCase().includes(part))) {
            return false;
        }
...
function modelSupportsNativeTools(modelDescription) {
    ...
    const providerSupport = exports.PROVIDER_TOOL_SUPPORT[modelDescription.provider];
    if (!providerSupport) {
        return false;
    }
