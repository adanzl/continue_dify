...
exports.postprocessCompletion = postprocessCompletion;
const lcs_js_1 = require("../../util/lcs.js");
const lineStream_js_1 = require("../filtering/streamTransforms/lineStream.js");
function rewritesLineAbove(completion, prefix) {
    ...
    return (0, lineStream_js_1.lineIsRepeated)(lineAbove, firstLineOfCompletion);
}
const MAX_REPETITION_FREQ_TO_CHECK = 3;
function isExtremeRepetition(completion) {
    const lines = completion.split("\n");
    if (lines.length < 6) {
        return false;
    }
    for (let freq = 1; freq < MAX_REPETITION_FREQ_TO_CHECK; freq++) {
        const lcs = (0, lcs_js_1.longestCommonSubsequence)(lines[0], lines[freq]);
...
function isOnlyWhitespace(completion) {
    const whitespaceRegex = /^[\s]+$/;
    return whitespaceRegex.test(completion);
}
function isBlank(completion) {
 ...
 */
function removeBackticks(completion) {
    const lines = completion.split("\n");
    if (lines.length === 0) {
        return completion;
    }
...
function postprocessCompletion({ completion, llm, prefix, suffix, }) {
    // Don't return empty
