...
const types_js_1 = require("../snippets/types.js");
// https://huggingface.co/stabilityai/stable-code-3b
const stableCodeFimTemplate = {
    ...
    compilePrefixSuffix: (prefix, suffix, filepath, reponame, snippets, workspaceUris) => {
        ...
        if (snippets.length === 0) {
            if (suffix.trim().length === 0 && prefix.trim().length === 0) {
                return [
                    `+++++ ${(0, uri_js_1.getLastNUriRelativePathParts)(workspaceUris, filepath, 2)}\n${prefix}`,
                    suffix,
                ];
            }
            return [prefix, suffix];
        }
        const relativePaths = (0, uri_js_1.getShortestUniqueRelativeUriPaths)([
...
const mercuryMultifileFimTemplate = {
    compilePrefixSuffix: (prefix, suffix, filepath, reponame, snippets, workspaceUris) => {
        function getFileName(snippet) {
            return snippet.uri.startsWith("file://")
                ? snippet.uniquePath
        ...
        if (snippets.length === 0) {
            if (suffix.trim().length === 0 && prefix.trim().length === 0) {
                return [
                    `<|file_sep|>${(0, uri_js_1.getLastNUriRelativePathParts)(workspaceUris, filepath, 2)}\n<|fim_prefix|>${prefix}`,
                    suffix,
                ];
            }
            return [`${prefix}`, suffix];
...
const starcoder2FimTemplate = {
    template: (prefix, suffix, filename, reponame, language, snippets, workspaceUris) => {
        const otherFiles = snippets.length === 0
            ? ""
            : `<file_sep>${snippets
                .map((snippet) => {
                return snippet.content;
...
const codegeexFimTemplate = {
    template: (prefix, suffix, filepath, reponame, language, allSnippets, workspaceUris) => {
        const snippets = allSnippets.filter((snippet) => snippet.type === types_js_1.AutocompleteSnippetType.Code);
        const relativePaths = (0, uri_js_1.getShortestUniqueRelativeUriPaths)([...snippets.map((snippet) => snippet.filepath), filepath], workspaceUris);
...
//# sourceMappingURL=AutocompleteTemplate.js.map