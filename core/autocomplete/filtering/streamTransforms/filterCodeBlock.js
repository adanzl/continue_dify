 ...
 */
async function* filterCodeBlockLines(rawLines, filepath) {
    // Collect all lines for analysis
    const allLines = await (0, markdownUtils_1.collectAllLines)(rawLines);
    // Check if it has nested markdown blocks (like ```markdown or ```md)
    const firstLine = allLines[0] || "";
    const hasNestedMarkdown = (0, lineStream_1.hasNestedMarkdownBlocks)(firstLine, filepath);
    ...
    const hasMarkdownHeaders = allLines.some((line) => line.trim().startsWith("#") && !line.trim().startsWith("```"));
    const hasCodeBlocks = allLines.some((line) => line.trim().startsWith("```") && line.trim().length >= 3);
    ...
    for (let i = 0; i < allLines.length; i++) {
        ...
        const nesting = (0, streamMarkdownUtils_1.processBlockNesting)(line, seenFirstFence, shouldRemoveLineBeforeStart);
        ...
        if (nestCount > 0) {
            ...
            const changedEndLine = (0, lineStream_1.shouldChangeLineAndStop)(line);
            if (typeof changedEndLine === "string") {
                ...
                if (hasNestedMarkdown &&
                    ...
                    if ((0, streamMarkdownUtils_1.shouldStopAtMarkdownBlock)(markdownStateTracker, i)) {
...
//# sourceMappingURL=filterCodeBlock.js.map