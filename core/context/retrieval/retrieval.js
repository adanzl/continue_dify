...
exports.retrieveContextItemsFromEmbeddings = retrieveContextItemsFromEmbeddings;
const uri_1 = require("../../util/uri");
const utils_1 = require("../providers/utils");
const NoRerankerRetrievalPipeline_1 = __importDefault(require("./pipelines/NoRerankerRetrievalPipeline"));
const RerankerRetrievalPipeline_1 = __importDefault(require("./pipelines/RerankerRetrievalPipeline"));
const DEFAULT_N_FINAL = 25;
async function retrieveContextItemsFromEmbeddings(extras, options, filterDirectory) {
    ...
    const tags = workspaceDirs.map((directory, i) => ({
        directory,
        branch: branches[i],
    }));
    const pipelineType = useReranking
        ? RerankerRetrievalPipeline_1.default
        : NoRerankerRetrievalPipeline_1.default;
    ...
    if (results.length === 0) {
        if (extras.config.disableIndexing) {
            void extras.ide.showToast("warning", "No results found (Indexing disabled).");
            return [];
        }
    ...
    return [
        ...
        ...results
            ...
            .map((r) => {
            const { relativePathOrBasename, last2Parts, baseName } = (0, uri_1.getUriDescription)(r.filepath, workspaceDirs);
