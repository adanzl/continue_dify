...
exports.requestFilesFromRepoMap = requestFilesFromRepoMap;
...
function isSupportedModel(config, modelTitle) {
...
async function requestFilesFromRepoMap(defaultLlm, config, ide, input, filterDirUri) {
    const llm = (0, util_1.getModelByRole)(config, "repoMapFileSelection") ?? defaultLlm;
    // Only supported for Claude models right now
    if (!isSupportedModel(config, llm.title)) {
        return [];
    }
    try {
        const repoMap = await (0, generateRepoMap_1.default)(llm, ide, {
...
This is the question that you should select relevant files for: "${input}"`;
        ...
        const content = (0, messageContent_1.renderChatMessage)(response);
        if (!content.includes("\n")) {
            return [];
        }
        ...
        const chunks = await Promise.all(filepaths.map(async (filepath) => {
            const uri = await (0, ideUtils_1.resolveRelativePathInDir)(filepath, ide);
            if (!uri) {
                return undefined;
            }
        ...
        return chunks.filter((c) => c !== undefined);
    }
    catch (e) {
...
//# sourceMappingURL=repoMapRequest.js.map