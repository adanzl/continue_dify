"use strict";
Object.defineProperty(exports, "__esModule", { value: true });
const index_js_1 = require("../index.js");
class DebugLocalsProvider extends index_js_1.BaseContextProvider {
    async getContextItems(query, extras) {
        // Assuming that the query is a number
        const localVariables = await extras.ide.getDebugLocals(Number(query));
        const threadIndex = Number(query);
        const thread = (await extras.ide.getAvailableThreads()).find((thread) => thread.id === threadIndex);
        const callStacksSources = await extras.ide.getTopLevelCallStackSources(threadIndex, this.options?.stackDepth || 3);
        const callStackContents = callStacksSources.reduce((acc, source, index) => `${acc}\n\ncall stack ${index}\n\`\`\`\n${source}\n\`\`\``, "");
        return [
            {
                description: "The value, name and possibly type of the local variables",
                content: `This is a paused thread: ${thread?.name}\n` +
                    `Current local variable contents: \n${localVariables}.\n` +
                    `Current top level call stacks: ${callStackContents}`,
                name: "Debugger",
            },
        ];
    }
    async loadSubmenuItems(args) {
        const threads = await args.ide.getAvailableThreads();
        return threads.map((thread) => ({
            id: `${thread.id}`,
            title: thread.name,
            description: `${thread.id}`,
        }));
    }
}
DebugLocalsProvider.description = {
    title: "debugger",
    displayTitle: "Debugger",
    description: "Local variables",
    type: "submenu",
    renderInlineAs: "",
};
exports.default = DebugLocalsProvider;
//# sourceMappingURL=DebugLocalsProvider.js.map