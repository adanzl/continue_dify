...
function getLastUserHistory(history) {
    ...
    if (Array.isArray(lastUserHistory.content)) {
        return lastUserHistory.content.reduce((acc, current) => {
...
const ReviewMessageCommand = {
    ...
    run: async function* ({ llm, history, abortController }) {
        ...
        for await (const chunk of llm.streamChat([{ role: "user", content: content }], abortController.signal)) {
            yield (0, messageContent_js_1.renderChatMessage)(chunk);
        }
    },
};
exports.default = ReviewMessageCommand;
