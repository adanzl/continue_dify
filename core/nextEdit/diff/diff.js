...
function checkFim(oldEditRange, newEditRange, cursorPosition) {
    ...
    const cursorOffset = oldEditLines
...
function calculateFinalCursorPosition(currCursorPos, editableRegionStartLine, oldEditRangeSlice, newEditRangeSlice) {
    ...
    const diffLines = (0, myers_1.myersDiff)(oldEditRangeSlice, newEditRangeSlice);
...
function applyCompletionToFile(fileContent, completion, startLineNumber, linesToReplace) {
    ...
    const numLinesToReplace = linesToReplace !== undefined ? linesToReplace : completionLines.length;
    // Replace the lines
    const newLines = [
 ...
 */
function groupDiffLines(diffLines, offset = 0, maxGroupSize) {
    const groups = [];
...
function processLimitedSizeGroup(diffLines, start, end, maxGroupSize, offset) {
    ...
    let oldContentLineEnd = oldContentLineStart +
