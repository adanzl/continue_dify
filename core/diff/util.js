 ...
 */
function matchLine(newLine, oldLines, permissiveAboutIndentation = false) {
 ...
 */
async function* streamLines(streamCompletion, log = false) {
    let allLines = [];
    let buffer = "";
    try {
        for await (const update of streamCompletion) {
            const chunk = typeof update === "string" ? update : (0, messageContent_js_1.renderChatMessage)(update);
            buffer += chunk;
...
async function* generateLines(lines) {
    for (const line of lines) {
        yield line;
