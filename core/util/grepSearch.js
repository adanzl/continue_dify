...
function formatGrepSearchResults(results, maxChars) {
    ...
    const processResult = (lines) => {
        ...
        if (changeIndentBy === 0) {
            ...
            keepLines.push(...resultContent.map((line) => line.substring(-changeIndentBy)));
        }
        else {
            keepLines.push(...resultContent.map((line) => " ".repeat(changeIndentBy) + line));
        }
    };
    let resultLines = [];
