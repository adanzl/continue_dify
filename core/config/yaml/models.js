...
exports.llmsFromModelConfig = llmsFromModelConfig;
const config_yaml_1 = require("@continuedev/config-yaml");
const llms_1 = require("../../llm/llms");
const AUTODETECT = "AUTODETECT";
function getModelClass(model) {
    return llms_1.LLMClasses.find((llm) => llm.providerName === model.provider);
}
...
async function modelConfigToBaseLLM({ model, uniqueId, llmLogger, config, isFromAutoDetect, }) {
    ...
    const mergedRequestOptions = (0, config_yaml_1.mergeConfigYamlRequestOptions)(rest.requestOptions, config.requestOptions);
    let options = {
        ...rest,
...
async function llmsFromModelConfig({ model, uniqueId, llmLogger, config, }) {
