...
exports.createNewPromptFileV2 = createNewPromptFileV2;
...
async function createNewPromptFileV2(ide, promptPath) {
    const workspaceDirs = await ide.getWorkspaceDirs();
    if (workspaceDirs.length === 0) {
        throw new Error("No workspace directories found. Make sure you've opened a folder in your IDE.");
    }
    const baseDirUri = (0, uri_1.joinPathsToUri)(workspaceDirs[0], promptPath ?? ".continue/prompts");
    // Find the first available filename
    let counter = 0;
    let promptFileUri;
    do {
        const suffix = counter === 0 ? "" : `-${counter}`;
        promptFileUri = (0, uri_1.joinPathsToUri)(baseDirUri, `new-prompt-file${suffix}.prompt`);
        counter++;
    } while (await ide.fileExists(promptFileUri));
    const globalContext = new GlobalContext_1.GlobalContext();
    const PROMPT_FILE = globalContext.get("hasAlreadyCreatedAPromptFile") === true
        ? DEFAULT_PROMPT_FILE
...
//# sourceMappingURL=createNewPromptFile.js.map