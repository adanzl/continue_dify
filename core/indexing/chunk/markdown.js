...
exports.cleanFragment = cleanFragment;
...
async function* markdownChunker(content, maxChunkSize, hLevel) {
    if ((0, countTokens_1.countTokens)(content) <= maxChunkSize) {
    ...
    if (hLevel > 4) {
        const header = findHeader(content.split("\n"));
        for await (const chunk of (0, basic_1.basicChunker)(content, maxChunkSize)) {
            yield {
                ...chunk,
                otherMetadata: {
                    fragment: cleanFragment(header),
    ...
    for (const section of sections) {
        for await (const chunk of markdownChunker(section.content, maxChunkSize - (section.header ? (0, countTokens_1.countTokens)(section.header) : 0), hLevel + 1)) {
            yield {
